...
- Phase 5.1 added an inactive standalone backend prompt-builder module at `backend/app/services/prompt_builder.py`; it deterministically formats caller-provided user message, conversation state, and preselected knowledge documents into bounded prompt text, but it is not yet wired into `ConversationManager`, retrieval, workflows, or any external API.
