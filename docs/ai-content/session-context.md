...
- Phase 6.1 adds a new inactive backend provider-neutral LLM Integration seam at `backend/app/llm/` with internal request/response models, provider/registry protocols, and an `LLMIntegrationService` facade. It remains fully disconnected from `ConversationManager`, `WorkflowEngine`, Vector-less RAG, Prompt Builder, FastAPI routes, and frontend chat flows.
