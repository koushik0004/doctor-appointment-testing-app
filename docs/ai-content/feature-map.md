...
- Current capability:
  ...
  - Exposes an inactive standalone Prompt Builder seam that now normalizes caller inputs into a canonical internal `PromptContext` model, validates that context deterministically, and only then formats already-selected context into bounded prompt text, without changing live chat behavior.
