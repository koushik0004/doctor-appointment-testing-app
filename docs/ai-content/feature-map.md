...
- Current capability:
  ...
  - Exposes an additional inactive provider-neutral LLM Integration seam under `backend/app/llm/` that defines future message/request/response contracts plus provider and registry boundaries without integrating any SDK or changing runtime routing.
...
- Limitation:
  ...
  - Knowledge retrieval is still backend-only and deterministic; it is not used by the inactive Prompt Builder seam, the inactive LLM Integration seam, embeddings, or a vector database, and it remains inactive while a workflow is running.
