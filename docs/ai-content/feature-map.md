...
- Current capability:
  ...
  - Exposes an additional inactive provider-neutral LLM Integration seam under `backend/app/llm/` that defines future message/request/response contracts plus provider translator, adapter, registry, generation-orchestration, provider-configuration, generation-budget, runtime-activation, and execution-policy boundaries; the canonical contract now also supports provider-neutral structured-output, tool-calling, reasoning, streaming, citation, provider/model metadata, multimodal-intent fields, and optional reusable generation-budget profiles, while inactive configuration can represent provider selection, model, key, base URL, timeout, retry, feature-flag, and profile-override settings without integrating any SDK or changing runtime routing.
