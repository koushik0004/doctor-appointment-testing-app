...
- Current capability:
  ...
  - Exposes an inactive standalone Prompt Builder seam that now normalizes caller inputs into a canonical internal `PromptContext` model, uses an internal Conversation Context Collector to normalize conversation state deterministically, validates that context, and only then formats already-selected context into bounded prompt text, without changing live chat behavior.
