...
- Current capability:
  ...
  - Exposes an additional inactive provider-neutral LLM Integration seam under `backend/app/llm/` that defines future message/request/response contracts plus provider translator, adapter, registry, generation-orchestration, and provider-configuration boundaries; the canonical contract now also supports provider-neutral structured-output, tool-calling, reasoning, streaming, citation, provider/model metadata, and multimodal-intent fields, while an inactive orchestrator can coordinate Prompt Builder output into canonical LLM generation and an inactive configuration layer can represent provider selection, model, key, base URL, timeout, retry, and feature-flag settings without integrating any SDK or changing runtime routing.
