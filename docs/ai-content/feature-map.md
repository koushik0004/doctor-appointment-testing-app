...
- Current capability:
  ...
  - Exposes an inactive standalone Prompt Builder seam that now normalizes caller inputs into a canonical internal `PromptContext` model, uses internal Conversation, Workflow, and Knowledge Context Collectors to normalize conversation state, workflow state, and already-selected documents deterministically, validates that context, and only then formats bounded prompt text without changing live chat behavior.
