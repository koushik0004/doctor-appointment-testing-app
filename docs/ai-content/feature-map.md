...
- Current capability:
  ...
  - Exposes an additional inactive provider-neutral LLM Integration seam under `backend/app/llm/` that defines future message/request/response contracts plus provider translator, adapter, registry, and generation-orchestration boundaries; the canonical contract now also supports provider-neutral structured-output, tool-calling, reasoning, streaming, citation, provider/model metadata, and multimodal-intent fields, while an inactive orchestrator can coordinate Prompt Builder output into canonical LLM generation without integrating any SDK or changing runtime routing.
