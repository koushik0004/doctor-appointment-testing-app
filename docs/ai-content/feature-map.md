...
- Current capability:
  ...
  - Exposes an inactive standalone Prompt Builder seam that now normalizes caller inputs into a canonical internal `PromptContext` model, uses internal Conversation and Workflow Context Collectors to normalize conversation and workflow state deterministically, validates that context, and only then formats already-selected context into bounded prompt text, without changing live chat behavior.
